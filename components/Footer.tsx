...
const Footer = () => {
  return (
    <footer className='bg-stone-900'>
      <footer className='flexCenter pb-24 pt-12'>
        <div className='padding-container max-container flex w-full flex-col gap-14'>
                ...
                <FooterColumn title={FOOTER_CONTACT_INFO.title}>
                  {FOOTER_CONTACT_INFO.links.map((link) => (
                    <Link
                      ...
                      <p className='whitespace-nowrap text-gray-10'>
                        {link.label}:
                      </p>
          ...
          <p className='regular-14 w-full text-center text-gray-10'>
            2024 GeekKeys | All rights reserved
          </p>
        </div>
      </footer>
    </footer>
...
const FooterColumn = ({ title, children }: FooterColumnProps) => {
  return (
    <div className='flex flex-col gap-5'>
      <h4 className='bold-18 whitespace-nowrap text-gray-10'>{title}</h4>
      {children}
    </div>
  );
};
